...
export class MenuFormPage extends FormModalBase<MenuModel> {
  ...
  form = buildForm(this.fb, [
    ...
    { name: 'noMenu', validators: [...V.required, ...V.maxLength(80)] }
  ]);

  private selById(c: MenuModel): void {
    this.menuService.selById(c)
      ...
      .subscribe({
        next: (response) => {
          if (this.action() === 'insert') {
            request.coMenu = response;
          }
          this.handleSuccess('Menú guardado exitosamente', request);
