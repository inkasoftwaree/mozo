...
{
    /// <summary>
    /// Descubre por reflexión toda clase que implemente <see cref="IEndpoint"/> en este
    /// ensamblado y la registra usando su <see cref="IEndpoint.Prefix"/> y el auto-tag.
    /// Reemplaza el registro manual de rutas en Program.cs.
    /// </summary>
    public static void MapEndpoints(this WebApplication app)
    {
        Type? endpointType = typeof(IEndpoint);

        IEnumerable<TypeInfo> endpoints = Assembly
        ...
        foreach (TypeInfo type in endpoints)
        {
            IEndpoint instance = (IEndpoint)Activator.CreateInstance(type.AsType())!;

            app.MapWithAutoTag(instance.Prefix, type.AsType(), instance.MapEndpoint);
        }
    }
