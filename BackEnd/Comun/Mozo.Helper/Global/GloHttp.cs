    ...
    private static readonly Dictionary<int, string[]> HttpStatusMap = new()
{
    ...
    {
        if (!code.HasValue)
            return new[] { "000", "Not determine", "not-determine" };

        int statusCode = code.Value;

        // Buscar código específico
        if (HttpStatusMap.TryGetValue(statusCode, out string[]? result))
            return result;
        ...
        return statusCode switch
        {
            ...
            >= 207 and <= 299 => new[] { "200", "Successful", "successful" },
            ...
            _ => new[] { "000", "Not determine", "not-determine" }
        };
    }
