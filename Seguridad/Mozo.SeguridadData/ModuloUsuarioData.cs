...
public interface IModuloUsuarioData
{
    ...
    Task<ModuloUsuarioModel?> SelByModuloAndUsuarioAsync(ModuloUsuarioFilterDto c);
}
public partial class ModuloUsuarioData : IModuloUsuarioData
{
    ...
    public async Task<int> InsertAsync(ModuloUsuarioModel c)
    {
        ...
        string sql = $@"SELECT {_schema}.fn_modulousuario_insert(
                @CoEmpresa,
                @CoUsuario,
                @CoModulo,
                @CoPerfil,
    ...
    public async Task UpdateAsync(ModuloUsuarioModel c)
    {
        ...
        string sql = $@"CALL {_schema}.usp_modulousuario_update(
            @CoModuloUsuario,
            @CoPerfil,
            @FeExpiracion
        )";
    ...
    public async Task UpdateConfiguracionAsync(ModuloUsuarioModel c)
    {
        DynamicParameters pr = new();       
        pr.Add2("CoModuloUsuario", c.CoModuloUsuario, DbType.Int32);
        pr.Add2("Valor", c.Valor, DbType.String);
        string sql = $@"CALL {_schema}.usp_modulousuario_update_configuracion(
            @CoModuloUsuario,@Valor)";
        await _connection.ExecuteScalarAsync(sql, pr);
    }
    public async Task DeleteByIdAsync(ModuloUsuarioFilterDto c)
    {
        ...
        pr.Add2("CoEmpresa", c.CoEmpresa, DbType.Int32);
        pr.Add2("CoModuloUsuario", c.CoModuloUsuario, DbType.Int32);
        string sql = $@"CALL {_schema}.usp_modulousuario_delete_by_id(
            @CoEmpresa,@CoModuloUsuario)";
        await _connection.ExecuteScalarAsync(sql, pr);
    }
    public async Task<IEnumerable<ModuloUsuarioModel>> SelAllByModuloAsync(ModuloUsuarioFilterDto c)
    {
        ...
        pr.Add2("CoEmpresa", c.CoEmpresa, DbType.Int32);
        pr.Add2("CoModulo", c.CoModulo, DbType.Int32);
        string sql = $@"SELECT * FROM {_schema}.fn_modulousuario_sel_all_by_modulo(
            @CoEmpresa,
            @CoModulo)";
    ...
    public async Task<IEnumerable<ModuloUsuarioModel>> SelAllByUsuarioAsync(ModuloUsuarioFilterDto c)
    {
        DynamicParameters pr = new();
        pr.Add2("CoUsuario", c.CoUsuario, DbType.Int32);
        string sql = $@"SELECT * FROM {_schema}.fn_modulousuario_sel_all_by_usuario(
            @CoUsuario
        )";
        return await _connection.QueryAsync<ModuloUsuarioModel>(sql, pr);
    }
    public async Task<ModuloUsuarioModel?> SelByIdAsync(ModuloUsuarioFilterDto c)
    {
        DynamicParameters pr = new();      
        pr.Add2("CoModuloUsuario", c.CoModuloUsuario, DbType.Int32);
        string sql = $@"SELECT * FROM {_schema}.fn_modulousuario_sel_by_id(
            @CoEmpresa,
            @CoModuloUsuario)";
        return await _connection.QueryFirstOrDefaultAsync<ModuloUsuarioModel>(sql, pr);
    }
    public async Task<ModuloUsuarioModel?> SelByModuloAndUsuarioAsync(ModuloUsuarioFilterDto c)
    {
        DynamicParameters pr = new();
        pr.Add2("CoEmpresa", c.CoEmpresa, DbType.Int32);
        pr.Add2("CoModulo", c.CoModulo, DbType.Int32);
        pr.Add2("CoUsuario", c.CoUsuario, DbType.Int32);
        string sql = $@"SELECT * FROM {_schema}.fn_modulousuario_sel_by_usuario_modulo(
            @CoEmpresa,
            @CoModulo,
            @CoUsuario
        )";
